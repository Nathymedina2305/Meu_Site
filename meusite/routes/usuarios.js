...
router.post('/cadastrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    if (senha == undefined || nome == undefined || apelido == undefined || email == undefined) {
	  // coloque a frase de erro que quiser aqui. Ela vai aparecer no formulário de cadastro
      throw new Error(`Dados de cadastro não chegaram completos: ${senha} / ${nome} / ${apelido} / ${email}`);
    }
    return banco.sql.query(`select count(*) as contagem from Login where email = '${email}'`);
  }).then(consulta => {
    ...
    res.status(500).send(erro);
	  ...
	  if (cadastro_valido) {		  
		...
		banco.sql.query(`insert into Login (nome, apleido, email, senha) values ('${nome}','${apelido}','${email}','${senha}')`).then(function() {
